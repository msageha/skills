...
[api-reference.md](api-reference.md). Base URL: `https://deco.msageha.net/api`.
...
```bash
curl -s "https://deco.msageha.net/api/devices" | jq '.[] | {
...
```bash
# Online clients only
curl -s "https://deco.msageha.net/api/clients?online_only=true" | jq '.[] | {
  name, ip, mac, connection_type, down_speed, up_speed
}'

# Blocked clients
curl -s "https://deco.msageha.net/api/clients/blocked" | jq '.[] | {name, mac}'
```
...
curl -s "https://deco.msageha.net/api/wireless" | jq 'to_entries
...
curl -s -X POST "https://deco.msageha.net/api/wireless" \
...
curl -s -X POST "https://deco.msageha.net/api/wireless/config" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "https://deco.msageha.net/api/network/wan" | jq '.wan | {dial_type, ip: .ip_info.ip}'
curl -s "https://deco.msageha.net/api/network/internet" | jq .
curl -s "https://deco.msageha.net/api/network/lan" | jq .
curl -s "https://deco.msageha.net/api/network/ipv6" | jq .
curl -s "https://deco.msageha.net/api/network/performance" | jq .
```

## Device Info

```bash
curl -s "https://deco.msageha.net/api/device/mode" | jq .
curl -s "https://deco.msageha.net/api/device/time" | jq .
curl -s "https://deco.msageha.net/api/cloud/device-info" | jq .
```
...
```bash
# All Deco units
curl -s -X POST "https://deco.msageha.net/api/reboot" \
  -H "Content-Type: application/json" \
  -d '{"confirm": true}' | jq .
...
```bash
curl -s -X POST "https://deco.msageha.net/api/raw" \
  -H "Content-Type: application/json" \
